...
source("../MdFOCuS_R_implementation/MdFocus_MeanVarGaussian_1d.R")               # MdFOCuS for Gaussian Model(change in mean and variance)
source("../MdFOCuS_R_implementation/MdFocus_MeanGaussian_md.R")                # MdFOCuS for Gaussian Model(change in mean)
...
colnames(data2) <- c('Number of matches', 'Mean', 'Mean and variance')
...
data <- data.frame(
  ...
  group = rep(c("Mean", "Mean and variance"), each = length(thrs_Mean))
)
################################################################################
...
#save plot
pdf(file = "figure/Figure_Treshold_by_training.pdf",  width = 5, height = 3.5) 
print(p_thrs)
dev.off()
...
#save plot
pdf(file = "figure/Figure_Time_detection_NBA_Application.pdf",  width = 5, height = 3.5) 
print(p_time_detect)
dev.off()
...
cs_data_ <- apply(data_, 2, cumsum)
nb_point <- sapply(10:nrow(cs_data_), FUN = function(n) length(unique(as.vector(convhulln(cs_data_[1:n, 1:2])))))
range(nb_point) ## 5 to 18 (compare to the number of lambda values used in e-detectors)
sort(unique(as.vector(convhulln(cs_data_[, 1:2]))))
## closest to 312 : 300, 304 and 351
## [1]   1   2  48  69 158 176 246 279 300 304 351 624 639 640
...
data <- data.frame(
  ...
  Game = c("Lost", "Won")[(x_[match_nb] > 0) + 1]
)
#get plot
arrow_change <- 279+.5
