...
name_mu_scl <- as.character(mu_scl)
name_lambda_scl <- as.character(lambda_scl)
...
Plot_f0 <- ggplot(table_f0, aes(x = table_f0[, 1], 
  ...
  scale_x_continuous(breaks= mu_scl,  
                     labels = name_mu_scl) +
  theme_bw() +
...
#save
pdf(file = "Figure_functions_fi_and_fmax_n3.pdf",  width = 3, height = 6)
plot_grid(Plot_f0, Plot_pr_f0max, ncol = 1, align = "v")
dev.off()
#save
pdf(file = "Figure_functions_fi_and_fmax_n4.pdf",  width = 3, height = 6)
plot_grid(Plot_f, Plot_pr_fmax, ncol = 1, align = "v")
dev.off()

#save
pdf(file = "Figure_functions_fi_and_fmax_n3_4.pdf",  width = 3, height = 10)
plot_grid(Plot_f0, Plot_pr_f0max, Plot_f,  Plot_pr_fmax, ncol = 1, align = "v")
dev.off()
...
Plot_pr_gmax <- ggplot(table_pr_gmax, aes(x = table_pr_gmax[, 1], 
  ...
  scale_x_continuous(breaks= mu_scl,  
                     labels = name_mu_scl) +
  scale_y_continuous(breaks= lambda_scl,  
                     labels = name_lambda_scl) +
  theme_bw() +
  theme(text = element_text(size = 13),
        legend.title = element_text(size = 13),
        legend.text=element_text(size = 13),
  ...
  labs(colour = "index i") 
pdf(file = "Figure_function_proj_gmax_n4.pdf",  width = 3, height = 3.5)
print(Plot_pr_gmax)
dev.off()
...
Plot_pr0_gmax <- ggplot(table_pr0_gmax, aes(x = table_pr0_gmax[, 1], 
  ...
  labs(colour = "index i") 
pdf(file = "Figure_function_proj_gmax_n3.pdf",  width = 3, height = 3.5)
print(Plot_pr0_gmax)
dev.off()

#save
pdf(file = "Figure_functions_proj_gmax_n3_4.pdf",  width = 2.3, height = 5.5)
plot_grid(Plot_pr0_gmax, Plot_pr_gmax, ncol = 1, align = "v")
dev.off()
