...
##    p-variate Independent Gaussian Model without change (p =1,..,5)         ##
################################################################################


# Test description--------------------------------------------------------|
# We generate ts ~ N(O,Ip) in dimension p = 1,.., 5                       | 
...
#' @param First_step (By default, (first step=1) + P_) 
#' @param Beta_  By default, Beta_ = 1
#' @param Alpha_  Value of alpha = 2
#'
#' @return returns time complexity for ts (without changes)
#' @example 
#' Runtime(cnst_=1,N_=1000,P_=2, Method_ = "FOCuS0", Cost_ = 'gauss')
