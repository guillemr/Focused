...
t_hat <- apply(focus0_mc, 2, quantile, probs = .47)
...
t_hat <- apply(md_focus0_part_mc, 2, quantile, probs = .45)
...
t_hat <- apply(focus0est_mc, 2, quantile, probs = .43)
t_multiplier <- cbind(focus0est_mc$max / t_hat["max"], focus0est_mc$sum / t_hat["sum"]) %>%
  apply(1, max) %>%
  quantile(probs = .43)
...
t_hat <- apply(md_focus_part_mc, 2, quantile, probs = .43)
