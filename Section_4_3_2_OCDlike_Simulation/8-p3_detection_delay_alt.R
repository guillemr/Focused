...
library(tidyr)
CORES <- 32
SERVER_ADDRESS <- "romano@ma-res-romano.lancaster.ac.uk"
# plan(multicore, workers = CORES)
# Set up the cluster plan with the SSH session
plan(cluster, workers = rep(SERVER_ADDRESS, CORES))
...
file <- paste0("Section_4_3_2_OCDlike_Simulation/results/det_del_", "p", p, "N", N, "_alt.RData")
if (file.exists(file)) {
...
md_focus0_part_res <- future_pmap(sim_grid, .f = function(delta, prop, changepoint, N, sim) {
  data <- t(generate_sequence(n = N, p = p, cp = changepoint, magnitude = delta, dens = prop, seed = sim)) # trasposing as the current prototype reads n x p (rather then p x n)
  res <- FocusCH(data, get_opt_cost = \(...) get_3_opts_reg(..., cost=cost_lr_partial0), threshold = thresholds$md_focus0_part)
  t <- which(res$nb_at_step == 0)[1]
  est <- if_else(is.na(t), N, t - 1)
...
tot_summ <- tot_res %>% filter(sim != 114) %>% mutate(dd = est - 500, density = density * p) %>% 
