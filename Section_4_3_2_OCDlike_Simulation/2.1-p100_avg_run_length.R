...
Y_nc <- future_map(1:REPS, function(i) generate_sequence(n = N, p = p, cp = 500, magnitude = 0, dens = 0, seed = i))
...
Y_monte_carlo <- future_map(1:REPS, function(i) generate_sequence(n = target_arl + 100, p = p, cp = 500, magnitude = 0, dens = 0, seed = i))
...
if (F) {
  ...
  t_hat <- apply(focus0est_mc, 2, quantile, probs = .46)
